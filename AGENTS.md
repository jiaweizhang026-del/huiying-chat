...
- 用户的 Figma 视觉是基准。2026-09-16 的 22–39/40/41 新稿覆盖旧配色：主体白底、浅灰聊天气泡、浅蓝云朵、浅绿玻璃边线；社区底色与收起星球的发现页保留浅灰。保留小圆角、悬浮导航、原始风景/头像/记忆照片，禁止擅自改成营销页、大卡片仪表盘或蓝紫 AI 风格。
...
- `styles.css` 根变量统一新版文字层级、表面色、玻璃边框和阴影；`public/assets/v4-*` 为新版 Figma 云朵/输入栏原件。闭合旁白正文40px高，展开随文字增长，不固定内容高度；旁白虚构说明保留为读屏文字。
- `npm run test:visual` 在独立5188生成 `test-results/v4-*.png`，验证新版颜色、尺寸、聚焦/失焦及五档宽度；与其他5188测试串行运行。
